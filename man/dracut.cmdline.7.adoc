...
**iso-scan/filename=**[__<devspec>__:]__<filepath>__::
    ...
    * *_<filepath>_* specifies the image file for loop mounting by _dracut_ to
    access the root filesystem within. +
+
NOTE: _Without_ the optional *_<devspec>_* argument, all mountable devices will
be mounted readonly in `/dev/disk/by-uuid/*` order. +
...
directory of the BOOT PARTITION OF THE HOST OPERATING SYSTEM so that GRUB
can address the isofile for loop setup and reach the linux kernel and initrds.
+
NOTE: The filepaths to the linux kernel and initrd below will differ depending
on the .iso's OS distribution and vintage. +
For example, Fedora 37-41 used paths `/images/pxeboot/vmlinuz` &
...
menuentry 'Live .iso' --class gnu-linux --class gnu --class os {
  ...
  linux (loop)/boot/x86_64/loader/linux iso-scan/filename="$isopath" root=live:iso-scan ro quiet splash
  initrd (loop)/boot/x86_64/loader/initrd
}
--
In the following example, the filesystem uuid of a storage partition
is set to the variable `dev_uuid`, enabling direct addressing of an isofile on
that filesystem.  On btrfs filesystems, the default root subvolume may need to
be included as an additional directory level for GRUB as indicated by
...
menuentry "Start 'Live .iso' --class gnu-linux --class gnu --class os {
  ...
  linux (loop)/boot/x86_64/loader/linux root=live:iso-scan \
    iso-scan/filename=UUID=${dev_uuid}:"$isopath" ro quiet splash
  initrd $initrds
  loopback -d loop
}
done
