...
_DRACUT_BINDIR_::
    overrides the directory containing Dracut executables used while building
    the initramfs, e.g. **dracut-install**. Used with **--sysroot**. If running
    **dracut** installed in a cross-compiled sysroot, use this option to
    provide host Dracut binaries.
+
Default:
    _$prefix/lib/dracut_
+
[NOTE]
===============================
If **DRACUT_INSTALL** is set, it takes precedence over **DRACUT_BINDIR**. If
you set both variables, ensure **DRACUT_INSTALL** contains the correct and full
path for **dracut-install**.
===============================
...
_DRACUT_MODULE_BUSYBOX_LINKS_::
    overrides the list of Busybox applets to link in the initramfs, must
    contain the path of a file containing the list to use. Used for
    **--sysroot** if the sysroot is cross-compiled and running `busybox --list`
    is not possible. The list may contain plain names or full paths.
+
    Using the **busybox.links** file produced by the Busybox build is
    recommended, using the output of `busybox --list` (e.g. running using Qemu
    userspace emulation) is also possible.
