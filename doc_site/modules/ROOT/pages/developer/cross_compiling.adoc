...
A few Dracut tools (`dracut-util`, optionally `skipcpio`) are used at runtime
(from the initramfs, or fully booted system). To be usable, they need to be
cross-compiled and installed into `$dracutsysrootdir/usr/lib/dracut/` (or see
**DRACUT_RUNTIMEDIR** in the dracut(8) man page).
